-- Phase 1: Critical Booking System Improvements (No Past Date Constraint)
-- Run this in your Supabase SQL Editor if you want to keep existing test data
...
-- 4. Add booking validation constraints (payment consistency only)
...
-- 5. Notification queue table removed - using direct webhook messaging to Make.com
...
-- 7. Create function to automatically log booking changes
...
-- 9. Create function to increment version on updates
...
-- 10. Create trigger for version increment
...
-- 11. Create view for admin booking history (includes audit trail)
...
-- 12. Create function for safe booking creation with race condition prevention
...
BEGIN
  ...
  FROM time_slots
  WHERE date = p_date AND time = p_time AND is_legacy = false
  FOR UPDATE;
...
SELECT 'Audit logging and constraints added successfully (no date constraint)!' as status;
